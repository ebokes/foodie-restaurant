...
export const userService = {
  ...
  updateAddresses: async (userId: string, addresses: UserAddress[]): Promise<void> => {
    try {
  ...
  // Add payment method
  addPaymentMethod: async (userId: string, paymentMethod: PaymentMethod): Promise<void> => {
    try {
      const userDoc = await getDoc(doc(db, 'users', userId));
      const currentMethods = userDoc.exists() ? userDoc.data().paymentMethods || [] : [];
      ...
      if (paymentMethod.isDefault) {
        ...
        await updateDoc(doc(db, 'users', userId), { paymentMethods: updatedMethods });
      } else {
  ...
  deletePaymentMethod: async (userId: string, paymentMethodId: string): Promise<void> => {
    try {
      const userDoc = await getDoc(doc(db, 'users', userId));
      if (userDoc.exists()) {
        const currentMethods = userDoc.data().paymentMethods || [];
        const updatedMethods = currentMethods.filter((pm: PaymentMethod) => pm.id !== paymentMethodId);
        await updateDoc(doc(db, 'users', userId), { paymentMethods: updatedMethods });
      }
    } catch (error) {
