...
const OrderProgressTimeline: React.FC<OrderProgressTimelineProps> = ({ timeline = [], currentStatus = 'order_confirmed' }) => {
  ...
  const getTimelineItemClass = (item: TimelineItem) => {
    if (item.completed && item.active) {
      return {
        ...
        icon: 'bg-linear-to-br from-primary-solid via-grad1 to-grad2 text-primary-foreground',
        title: 'text-primary',
      ...
      <div className="hidden md:block">
        <div className="relative">
          ...
          <div className="absolute top-6 left-0 right-0 h-1 bg-muted rounded-full">
            <div 
              className="h-full bg-linear-to-br from-primary-solid via-grad1 to-grad2 rounded-full transition-all duration-500 ease-out"
              style={{
            ...
            {timeline.map((item, index) => {
              ...
              return (
                <div key={item.status} className="flex flex-col items-center text-center max-w-[140px]">
                  ...
                  {/* Active Indicator */}
                  {item.active && (
                    <div className="absolute -top-1 -right-1 w-3 h-3 bg-linear-to-br from-primary-solid via-grad1 to-grad2 rounded-full animate-pulse"></div>
                  )}
                </div>
              );
            })}
      ...
      <div className="mt-6 pt-4 border-t border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="w-2 h-2 bg-linear-to-br from-primary-solid via-grad1 to-grad2 rounded-full animate-pulse"></div>
