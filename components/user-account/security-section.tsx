...
const SecuritySection: React.FC<SecuritySectionProps> = ({ onUpdatePassword, onUpdateSecurity }) => {
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (passwordErrors[name as keyof PasswordErrors]) {
      setPasswordErrors(prev => {
        ...
        delete newErrors[name as keyof PasswordErrors];
        return newErrors;
      });
    }
  };
  ...
  const handleSecurityToggle = (setting: keyof SecuritySettings, value: boolean | string): void => {
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
                  securitySettings.twoFactorEnabled ? 'bg-linear-to-br from-primary-solid via-grad1 to-grad2' : 'bg-muted'
                }`}
              >
                <span
          ...
          <div className="p-4 border border-border rounded-lg bg-background">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
                  securitySettings.loginNotifications ? 'bg-linear-to-br from-primary-solid via-grad1 to-grad2' : 'bg-muted'
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
