...
const AddressInfoSection: React.FC<AddressInfoSectionProps> = ({ addresses, onUpdateAddresses }) => {
    ...
    if (validateForm()) {
      ...
      if (editingAddressId) {
        ...
        updatedAddresses = addresses.map(addr => 
          addr.id === editingAddressId ? { ...formData, id: editingAddressId } as Address : addr
        );
      } else {
        // Add new address
      ...
      if (formData.isDefault) {
        updatedAddresses = updatedAddresses.map(addr => ({
          ...addr,
          isDefault: addr.id === editingAddressId || addr.id === updatedAddresses[updatedAddresses.length - 1].id
        }));
      }
